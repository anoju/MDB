...
$(function(){
	...
	if($('.ui_spl_txt').length)$('.ui_spl_txt').splitText();
...
var common = {
	...
	gnb:function(){
		//var $evtType = 'click';
		//if(isMobile.iOS())$evtType = 'touchend';
	...
	//스킵네비 삽입
		...
		if($('#content').length && !$('#skipNavi').length)$('body').prepend($naviHtml);
	},
	meta: function(){
...
var buttonUI ={
	...
	effect: function(){
		//버튼 클릭 효과
		var btnInEfList = 'a.btn, button.btn,.accordion .title a, .product_item>a, .check_list_line .chk label, .direct_link li a, .bn_link_item2>a, .bn_link_item3>a, .bn_link_item4>a, .bn_link_item5>a, .link_item>a';
...
var formUI = {
	...
	etc:function(){
		...
		$swichBtn.each(function(){
			if($(this).prop('checked')){
				$(this).next('label').find('.offscreen').text('설정등록');
			}else{
				$(this).next('label').find('.offscreen').text('설정해제');
			}
		});
...
var mapResize = function(){
	$(window).resize(function(){
		$('.map.full').each(function(){
			var $mapTop = $(this).position().top,
				$closest = $(this).closest('.map_wrap'),
				$mapWrapTop = $closest.position().top + parseInt($closest.css('margin-top')),
				$mapHeight = $(window).height() - $mapTop - $mapWrapTop;
...
$.fn.rollingNumber = function(number){
	return this.each(function(){
		...
		for(var i=0; i<$split.length; i++){
			if($split[i] == ','){
				$this.append('<span class="rl_num_comma">'+$split[i]+'</span>');
			}else{
				$this.append('<span class="rl_num_item" style="height:'+$height+'px;line-height:'+$height+'px;"><span class="rl_num" style="height:'+$height+'px;line-height:'+$height+'px;"></span></span>');
				for(var j=$repeatNum;j>=0;j--){
				//for(var j=0;j<=$repeatNum;j++){
					var k = j%10;
					$this.find('.rl_num_item').last().find('.rl_num').append('<span class="rl_span" style="height:'+$height+'px;line-height:'+$height+'px;">'+k+'</span>');
				}
			}
		}
		...
		$(window).on('resize',function(){
			...
			$this.find('.rl_num_item, .rl_num_item > .rl_num, .rl_num_item > .rl_num > .rl_span').css({
				'height':'auto',
				'line-height':'inherit'
			});
			$height = $this.find('.rl_num_item > .rl_num > .rl_span').height();
			$this.find('.rl_num_item, .rl_num_item > .rl_num, .rl_num_item > .rl_num > .rl_span').css({
				'height':$height+'px',
				'line-height':$height+'px'
			});
			$this.children().each(function(e){
				var $rol = $(this).find('.rl_num'),
					$num = $split[e];
...
//숫자 애니메이션2
//$('.number').transNumber('123,000');
$.fn.transNumber = function(chageNum){
	return this.each(function(){
		if(chageNum == "" || chageNum == undefined)chageNum = $(this).text();
		var $this = $(this),
			$style = $this.attr('style'),
			$height = $this.height(),
		...
		for(var i=0; i<$split.length; i++){
			if($split[i] == ','){
				...
				$this.append('<span class="rl_num_item">'+$split[i]+'</span>');
			}
		}
		...
		if(chageNum != nowNum){
			if(nowNum.length > chageNum.length){
				...
				for(var j in $delSplit){
					if($delSplit[j] != ','){
						$this.append('<span class="rl_num_item rl_num_out">'+$delSplit[j]+'</span>');
					}
				}
			}else{
				//숫자길이가 커질때
				if((chageNum.length - nowNum.length) > 1){
					$this.children('.rl_num_item').eq(nowNum.length-1).nextAll().not('.rl_num_comma').addClass('rl_num_in');
				}else{
					$this.children().last().addClass('rl_num_in');
				}
			}
			...
			if($style)$this.attr('style',$style);
			setTimeout(function(){
				$this.find('.rl_num_in, .rl_num_out').addClass('show');
				$this.find('.rl_num_out').animate({'width':0},500,function(){
					$(this).remove();
				});
			},100);
		}
		...
		for(var i=0; i<$split.length; i++){
			if($isTag){
				...
				if($split[i] == '<'){
					...
					$html += '<span class="spl_item">&nbsp;</span>';
				}else{
					j++;
					$html += '<span class="spl_item" style="';
...
//타이핑 애니메이션2: br 태그만 사용가능
//data-animation="on" 과 같이 사용가능
//$('.txt').typingText();
$.fn.typingText = function(){
	return this.each(function(){
		var $this = $(this),
			...
			$speed = $this.data('speed'),
			$delay = $this.data('delay'),
			$isCursor = $this.data('cursor'),
			$cursor = '<span class="tp_cursor" aria-hidden="true">|</span>',
			$html = '',
			isOn = $this.data('animation');
		...
		var typingAni = function(){
			setTimeout(function(){
				if($isCursor == true)$this.append($cursor);
				for(var i = 0; i < $split.length; i++){
					(function(i,char){
						setTimeout(function(){
							$html += char;
...
var mainUI = {
	banner: function(){
		...
		// var $circleLineOn = function(idx){
		//	var $max = 630,
		//		$svg = $('.main_banner .banner_img .circle svg circle'),
		//		$dashoffset = parseInt($svg.css('stroke-dashoffset')),
		//		$circleVal = $max - ($max * (idx/$slideLength));
		//	if($dashoffset == 0 && $circleVal >= ($max/2)){
		//		$svg.css({
		...
		//		});
		//		setTimeout(function(){
		//			$svg.removeAttr('style').css('stroke-dashoffset',$circleVal);
		//		},10);
		//	}else if($dashoffset >= ($max/2) && $circleVal == 0){
		//		$svg.removeAttr('style').css('stroke-dashoffset',$max);
		//		setTimeout(function(){
		//			$svg.css({
		//				'-webkit-transition-duration':'0s',
		//				'transition-duration':'0s',
		...
		//	$($imgCircle).find('.img').eq(idx-1).addClass('on').siblings().removeClass('on');
		// };
		var isChange = false;
		var decoNum = 1;
		var decoMax = 3;
		var $mainBanner = new Swiper($container,{
			...
			on: {
				...
				touchMove:function(e){
					...
					//$($imgCircle).dequeue().css('left',$diff);
				...
				transitionEnd:function(e){
					//var $left = parseInt($($imgCircle).css('left'));
					//if($left != 0)$($imgCircle).stop(true,false).animate({'left':0},450,'easeOutCubic');
