	...
	$.Mk.create('Datepicker', {
		...
		_buildOptions: function(options) {
			...
			for(var t in this._templates) {
				...
				this._templates[t] = this.options['template_' + t] || this._templates[t];
			}
		},

		_init: function($target, options) {
			this.$target = $($target);
			if(this.$target.is('input'))this.$target.attr('autocomplete', 'off');
		...
		_buildDisplay: function() {
			...
			var _lblVal = this._label().split('. '),
				_lblYear = _lblVal[0],
				_lblMmonth = _lblVal[1].replace('월','');
			if(_lblMmonth.length == 1)_lblMmonth = '0'+_lblMmonth;
			$d.append('<input type="month" value="'+_lblYear+'-'+_lblMmonth+'" title="달력 년월 변경" aria-hidden="true" tabindex="-1">');
		...
		_bind: function() {
			...
			if (!this.options.inline) {
				$(document.body).on(this._ns('mousedown'), function(e) {
					me._clickoff(e);
				}).on(this._ns('mousedown'),'.mk-datepicker',function(e){
					e.stopPropagation();
				});
				
				$(document.body).on(this._ns('focusin'), function(e){
					me._clickoff(e);
				}).on(this._ns('focusin'),'.mk-datepicker', function(e){
					e.stopPropagation();
				});
			}
			...
			this.$calendar.on('change','input[type=month]', function(e) {
				if($(this).val() != ''){
					var _ipVal = $(this).val().split('-');
					return me._showInputDay(_ipVal[0],_ipVal[1]);
				}
			});
		},
		...
		_keydownEnter: function(e, $curDay) {
			...
			this.$target.prev('input').val(date).change();
			this.$target.prev('input').trigger(this._ns('change'), [date]);
			this.$target.focus();
		...
		_clickCalendar: function(e) {
			...
			this.$target.prev('input').val(this.dateToString(this.date)).change();
			this.$target.prev('input').trigger(this._ns('change'), [this.date]);
			this.$target.focus();
		...
		refresh: function(day, month, year) {
			...
			var _lblVal = this._label().split('. '),
				...
				_lblMmonth = _lblVal[1].replace('월','');
			if(_lblMmonth.length == 1)_lblMmonth = '0'+_lblMmonth;
			this.$calendar.find(this._class('label', true)).text(this._label()).append('<input type="month" value="'+_lblYear+'-'+_lblMmonth+'" title="달력 년월 변경" aria-hidden="true" tabindex="-1">');
		},

		show: function() {
			if(!$('.mk-dimmed').length){
				this.$container.before('<div class="mk-dimmed" aria-hidden="true"></div>');// publishing add
			}
			if (!this.options.inline) {
